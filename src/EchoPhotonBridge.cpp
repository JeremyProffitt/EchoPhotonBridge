...
int EchoPhotonBridge::callEchoFunctionV2(String value)
{
    ...
    case DEVICETYPEV2_ONOFF:
        if (_v2devices[device].fOnOff == NULL)
            return -1;
        on = (_stoi(params) > 0);
    ...
    case DEVICETYPEV2_PERCENT:
        if (_v2devices[device].fPercent == NULL)
            return -1;
        delta = _getDelta(params, _v2devices[device].percent);
        _v2devices[device].percent += delta;

        if (_v2devices[device].percent > 100)
            _v2devices[device].percent = 100;
        if (_v2devices[device].percent < 0)
            _v2devices[device].percent = 0;

        return _v2devices[device].fPercent(device, _v2devices[device].percent, delta, params);

    case DEVICETYPEV2_COLOR:
        if (_v2devices[device].fColor == NULL)
            return -1;
    ...
    case DEVICETYPEV2_LOCK:
        if (_v2devices[device].fLock == NULL)
            return -1;
    ...
    case DEVICETYPEV2_TEMP:
        if (_v2devices[device].fTemp == NULL)
            return -1;
    ...
    case DEVICETYPEV2_LIGHTTEMP:
        if (_v2devices[device].fLightTemp == NULL)
            return -1;
...
int EchoPhotonBridge::callEchoFunctionV3(String value)
{
    ...
    case DEVICETYPEV3_VOLUMEPERCENT:
        ...
        if (_v3devices[device].volumePercent > 100)
            _v3devices[device].volumePercent = 100;
        if (_v3devices[device].volumePercent < 0)
            _v3devices[device].volumePercent = 0;
    ...
    case DEVICETYPEV3_VOLUMESTEP:
        if (_v3devices[device].fVolumeStep == NULL)
            return -1;
        muted = (_stoi(_getValue(params, ',', 0))) == 1;
                                       ...
                                       functionLightTemp fLightTemp)
{
    ...
    if (_echoDeviceCountV2 > MAX_ECHO_DEVICESV2)
        return;
                                       ...
                                       String commaSeperatedInputList)
{
    if (_init_required)
        init();
    //Check if we have enough devices available to use.
    if (_echoDeviceCountV3 > MAX_ECHO_DEVICESV3)
        return;
...
int EchoPhotonBridge::_getIndex(String data, String csvData, char separator)
{
    ...
    for (int i = 0; i <= maxIndex; i++)
    {
        if (csvData.charAt(i) == separator || i == maxIndex)
        {
            strIndex[0] = strIndex[1] + 1;
            strIndex[1] = (i == maxIndex) ? i + 1 : i;
            if (csvData.substring(strIndex[0], strIndex[1]).toUpperCase() == data.toUpperCase())
                return index;
            index++;
        }
    }
